...
| U4 | default detection | **A REAL FLAG EXISTS.** `systemDefault` is present on 14 of 25 entities. The earlier claim that no entity has one was wrong. |
...
| U8 | Channels | **RESOLVED - Channels ARE entry points** with a non-`TELEPHONY` `channelType`. The earlier "no API" claim was wrong. |
| U9 | entry-point listing completeness | **DEFECT - the unfiltered listing hides `systemInternal` rows** (10 vs 11). Sweep every `channelType`. |
| U10 | missing entities | `contact-number`, `dial-plan`, `agent-personal-greeting` were absent from the registry; two held real data. |
| U11 | registry correctness | 4 create fields and 3 dependencies were wrong. The `deps` errors were load-bearing; `create` is documentation only. |
...
25 entities.** It never appeared in the OpenAPI schemas I searched, but the API
returns it.
...
## U8 - Channels: RESOLVED (they were never missing)

**CORRECTION.** This file, the README, the user guide and every generated
`UNSUPPORTED.md` claimed Channels had no API and had to be recreated by hand.
**That was wrong**, and wrong in the worst direction: it told users their
configuration was lost when it was in the archive all along.

The error came from searching for **paths and tags named `channel`**, which
returns nothing - because a Channel is not a separate resource.

**A Channel is an entry point whose `channelType` is not `TELEPHONY`.**

`EntryPointDTO` (30 fields) carries:

| field | enum / note |
|---|---|
| `channelType` | `TELEPHONY, EMAIL, FAX, CHAT, VIDEO, OTHERS` in the DTO; the queue DTO additionally publishes `SOCIAL_CHANNEL, CUSTOM_MESSAGING, WORK_ITEM` |
| `socialChannelType` | `MESSAGEBIRD, MESSENGER, WHATSAPP, APPLE_BUSINESS_CHAT, GOOGLE_BUSINESS_MESSAGES` |
| `assetId`, `subscriptionId`, `imiOrgType` | digital-channel wiring (`imiOrgType`: `MIXED_MODE`, `IMI`) |
| `routePointId`, `controlFlowScriptUrl`, `overflowNumber`, `xspVersion` | unset on every row of the probed tenant |

On `davidwolgast-8xgo` all 11 entry points were `TELEPHONY`, and every
`?channelTypes=<digital>` query returned `200` with zero items - so the union
logic is **exercised by tests, not by real digital-channel data**. Its queues
did carry `EMAIL` and `SOCIAL_CHANNEL`, so digital channels were partly
configured without digital entry points.

---

## U9 - the entry-point listing is incomplete by default

```
GET organization/{orgId}/v2/entry-point                        -> 10 rows
GET organization/{orgId}/v2/entry-point?channelTypes=TELEPHONY -> 11 rows
```

The extra row is `Record_Agent_Greeting`, `systemInternal: true`,
`systemDefault: true`. **The unfiltered listing silently omits `systemInternal`
objects; passing an explicit `channelTypes` filter bypasses that.**

`export_cc._list_entity` now sweeps every declared `channelType` and unions by
id, keeping the plain listing first so a row the sweep misses is still kept,
and tolerating one filter value failing without losing the others. Driven by a
`list_sweep` spec in the registry, so no other entity pays the extra calls.

**Unknown:** whether any other entity's listing hides rows the same way. Only
entry-point was tested.

---

## U10 - three entities were missing from the registry

Found by diffing this registry against the sibling `wxcc-skills` one on
2026-09-23, hours before the sandbox expired:

| entity | rows on the live tenant | note |
|---|---|---|
| `contact-number` | **1** (`5598621`, user-created) | **no `name` field** - identity is `number`. Collision detection would have read every row as nameless and skipped it. |
| `dial-plan` | **2** (`US`, `Any Format`) | both `systemDefault: true` |
| `agent-personal-greeting` | 0 | **record shape and binary route UNVERIFIED** - carries audio like `audio-file`; probe before trusting |

`registry.name_field()` now declares the identity field per entity.

---

## U11 - the registry was wrong in seven places

Found with `scripts/capture_shapes.py`, which snapshots every entity's real
field inventory, types, sample values and reference graph.

### Dependencies - LOAD-BEARING (`plan.order_entities` reads these)

| was | now | evidence |
|---|---|---|
| `entry-point` depended on `dial-number` | **inverted**: `dial-number` depends on `entry-point` | a dial-number carries `entryPointId`; `entry-point/{id}/incoming-references` -> `['dial-number']` |
| `user-profile` had no deps | depends on `resource-collection` | `resource-collection/{id}/incoming-references` -> `['user-profile']` |
| `outdial-ani` had no deps | depends on `dial-number` | `dial-number/{id}/incoming-references` -> `['flow', 'outdial-ani']` |

The inversion would have created every dial number before its entry point
existed, dangling the reference.

### Create fields - DOCUMENTATION ONLY

**A correction to an earlier claim of mine:** I wrote that the wrong field
names "would 400 every create." **They would not.** Nothing in `src/` reads
`create`; the importer sends the whole exported record minus identity fields.
The wrong values were misleading documentation, not a runtime fault.

| entity | I had | actually |
|---|---|---|
| `business-hours` | `timeZone` | `timezone` |
| `skill` | `type` | `skillType` |
| `dial-number` | `number` | `dialledNumber` |
| `skill-profile` | `active` | no such field |

14 of 25 lists were also incomplete. All now come from the sibling registry,
which was built with real writes against a live tenant.

**Provenance correction:** `registry.py` claimed its facts were "cross-checked
against the live-verified registry in the sibling wxcc-skills repo." For the
create lists that cross-check never happened - the sibling had all four values
right, and its `dial-number` entry even lists `entryPointId`, which is exactly
the dependency direction this project had backwards.

### Observed reference graph

```
agent-profile         <- user
audio-file            <- flow
business-hours        <- resource-collection
cad-variable          <- SUBFLOW, flow, resource-collection
contact-service-queue <- flow
dial-number           <- flow, outdial-ani
entry-point           <- dial-number
holiday-list          <- business-hours, resource-collection
multimedia-profile    <- user
outdial-ani           <- desktop-profile
overrides             <- resource-collection
resource-collection   <- user-profile
site                  <- team, user
skill                 <- contact-service-queue, skill-profile
skill-profile         <- user
user                  <- team
work-type             <- auxiliary-code
```

Read as "X is pointed at by Y", so X must be created first.
